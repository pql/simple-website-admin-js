...
export const importArrayBlock = (list, group) => {
  list.forEach(ele => {
    ele.group = group;
    ele.index = uuid();
    if (ele.children) importArrayBlock(ele.children, group)
  })
}
...
export const getComparisonFunction = (operator) => {
  switch (operator) {
    ...
    case '<=': return (a, b) => a <= b;
    case '>=': return (a, b) => a >= b;
    default: throw new Error("不支持的比较操作");
  }
}
...
export const switchTheme = (value, safe) => {
  if (!value) value = document.body.dataset.theme === 'light' ? 'dark' : 'light';
  safe.$loadingParams['element-loading-background'] = themeObj[value].loadBackground
