<template>
  <div class="contentmenu"
    ...
    <div class="contentmenu__item"
      ...
      交互
      <i class="el-icon-caret-right  contentmenu__list--icon"></i>
      <div class="contentmenu contentmenu__list"
           @click="hover2=false"
           v-if="hover2">
        <div class="contentmenu__item"
             v-if="contain.validProp('dataList')"
             @click="handleOpen('data')"><i class="el-icon-document-copy"></i>
          添加数据
        </div>
        <div class="contentmenu__item"
             @click="handleOpen('transfer')"><i class="el-icon-thumb"></i>
          添加交互
        </div>
        <div class="contentmenu__item"
             @click="handleOpen('mouse')"><i class="el-icon-mouse"></i>
          添加事件
        </div>
        <div class="contentmenu__item"
             v-if="contain.validProp('dataList')"
    ...
    <div class="contentmenu__item"
      ...
      <div class="contentmenu contentmenu__list"
           ...
           v-if="hover">
        <div class="contentmenu__item"
             @click="handleSc()"><i class="el-icon-star-off"></i>
          收藏组件
        </div>
        <div class="contentmenu__item"
             v-if="['html','vue','common'].includes(contain.activeComponent.prop)"
             @click="handleImportBlock()"><i class="el-icon-upload"></i>
          导出组件库
        </div>
        <div class="contentmenu__item"
...
import { createFile, uuid } from '@avue/avue-data/utils/utils'
import { addObj } from '@avue/avue-data/api/components'
import { getList as getModuleList } from "@avue/avue-data/api/category";
export default {
  name: 'contentmenu',
  inject: ["contain"],
  data () {
  ...
  created () {
    ...
    // getModuleList({
    //   categoryModule: 'component',
    //   current: 1,
    //   size: 100,
    // }).then(res => {
    //   const data = res.data.data;
    //   this.DIC.module = data.map(ele => {
    //     return {
    //       label: ele.categoryKey,
    //       value: ele.categoryValue
    //     }
    //   })
    // });
  },
  methods: {
    ...
    handleImportBlock () {
      let result = {
        vue: {
          type: 0,
          content: this.contain.activeOption.content
        },
        echart: {
          type: 1,
          content: this.contain.activeObj.echartFormatter
        },
        html: {
          type: 2,
          content: this.contain.activeOption.content
