        ...
        <el-collapse v-model="tabs">
          <el-collapse-item :name="index"
            ...
            <template slot="title">
              ...
              <i class="transfer__close el-icon-close"
                 @click.stop="delParamsList(index,true)"></i>

            </template>
            <div class="transfer__list">
              <el-form-item label-width="0">
                <div class="avue-collapse transfer__box">
                  <div class="header">
                    满足条件
...
export default {
  ...
  computed: {
    isList () {
      return this.contain.activeObj.transfer.paramList && this.contain.activeObj.transfer.paramList.length !== 0
    }
  },
  methods: {
    handleCopy () {
    ...
    handlePaste () {
      ...
      this.contain.activeObj.transfer.paramList.push({
        name: '状态场景' + (this.list.length + 1),
        switch: true,
   ...
   return true
    ...
    open (index) {
      this.list = this.deepClone(this.contain.activeObj.transfer.paramList)
      this.tabs = index
      this.box = true;

    },
    submit () {
      this.$set(this.contain.activeObj.transfer, 'paramList', this.list)
      this.close()
    },
    close () {
      this.box = false;
    ...
    getTitle (item, index) {
      return `${index + 1}.${item.name}__${this.getTypeLabel(item.type)}`
    },
    getTypeLabel (value) {
    ...
    delParamsList (index, flag) {
      if (flag) {
        ...
        this.contain.activeObj.transfer.paramList.splice(index, 1)
      }
