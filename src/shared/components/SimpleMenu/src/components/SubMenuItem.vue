...
import { CollapseTransition } from "@/shared/components/Transition";
import {
  defineComponent,
  defineProps,
  getCurrentInstance,
...
} = inject(`subMenu:${getParentMenu.proxy._uid}`);
...
function handleMouseleave(deepDispatch = false) {
  const parentName = getParentMenu.value && getParentMenu.value.props.name;
  if (!parentName) {
  ...
  if (deepDispatch) {
    if (getParentSubMenu.value) {
      parentHandleMouseleave && parentHandleMouseleave(true);
    }
  }
}
...
onBeforeMount(() => {
  ...
  rootMenuEmitter.on("on-update-opened", (data) => {
    ...
    if (isObject(data) && rootProps.accordion) {
      const { opend, parent, uidList } = data;
      if (parent === instance.proxy.parent) {
        state.opened = opend;
      } else if (!uidList.includes(instance.proxy._uid)) {
        state.opened = false;
      }
      return;
    }
  ...
  rootMenuEmitter.on("on-update-active-name:submenu", (data) => {
    if (instance.proxy._uid) {
      state.active = data.includes(instance.proxy._uid);
    }
  });
});
