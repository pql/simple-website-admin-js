...
import { ADD_AJAX_ERROR_INFO, USER_LOGOUT_ACTION } from '@/shared/store/mutation-types';
...
const transform = {
    ...
    transformResponseHook(res, options) {
        ...
        // 这里逻辑可以根据项目进行修改
        const hasSuccess = status >= 200 && status < 300
        if (hasSuccess) {
...
function createAxios(opt) {
    ...
    return new VAxios(
        ...
        deepMerge({
            ...
            timeout: 30 * 1000,
            // 基础接口地址
            baseURL: remoteServiceBaseUrl || globSetting.apiUrl || '/',
